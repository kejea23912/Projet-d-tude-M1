---
title: "Présentation Datamining et datavisualisation"
author:
- "Kenny Jean-elie"
- "Ibrahima Caba Bah"
- "Fatou Diop Ndeye"
...
```{r}
fviz_ca(res.CA, axes = c(1, 2), repel = TRUE) +
  theme_minimal() 
```
## Synthèse finale 

```{r}
fviz_ca(res.CA, axes = c(3, 4), repel = TRUE) +
  theme_minimal()
...
## Inertie 
```{r}
fviz_eig(resMCA , addlabels =TRUE)
```
...
On commence par faire par faire une classification hiérarchique ascendante sur la sortie de l'ACM avec consolidation puis faire la méthodes **k-means** 
...
df[c(par1, par2, par3), ] |>
  ...
  pack_rows(
    "Cluster 3", length(par1) + length(par2) + 1, 
...
df[c(dist1, dist2, dist3), ] |>
  ...
  pack_rows(
    "Cluster 3", length(dist1) + length(dist2) + 1, 
    length(dist1) + length(dist2) + length(dist3)
  )
```
...
res.HCPC_consol$desc.var$category$`3` |> 
  kable(digits = c(1,1,1,200,2), format = "latex", booktabs = TRUE) |> 
  kable_styling(font_size = 8, latex_options = c("scale_down", "HOLD_position"))
```
